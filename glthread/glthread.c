...
void gltherad_add_before(glthread_t *curr_glthread, glthread_t *new_glthread){
...
get_glthread_list_count(glthread_t *base_glthread){
	...
	ITERATE_GLTHREAD_BEGIN(base_glthread, glthreadptr){
		count++;
	} ITERATE_GLTHREAD_END(base_glthread, glthread);
	return count;
...
#if 0
void *
gl_thread_search(glthrad_t *base_glthread,
